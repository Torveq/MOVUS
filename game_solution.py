...
class App:
    ...
    def __init__(self, root):
        ...
        self.GameFont = tkinter.font.Font(
            family="CyberpunkCraftpixPixel", size=16
        )  # additional options weight, underline, overstrike, slant, etc
        self.InputFont = tkinter.font.Font(
            family="CyberpunkCraftpixPixel", size=67
        )
        self.LBFont = tkinter.font.Font(
            family="CyberpunkCraftpixPixel", size=10
        )
        ...
        # (NPCWR is an abbreviation for NPC Walk Right
        # BR is bite right AR is attack right etc..)
        ...
        self.run_attack_right = self.load_frames(
            "Assets\RightRunAttack"
        )
    ...
    def start_menu(self):
        ...
        self.load_settings()
        # Create a frame for the start menu
        # (with dimesniosn equal to that of the image)
        ...
        self.button_img = ImageTk.PhotoImage(
            Image.open("Assets\PlayButton.png")
        )
        self.play_button = Button(
            self.start_frame,
            image=self.button_img,
            command=self.username_entry,
            bd=0,
        ...
        self.play_button.bind(
            "<Return>", lambda event: self.play_button.invoke()
        )  # Bind Enter key to play button
        self.play_button.place(relx=0.5, rely=0.715, anchor=CENTER)

        # Leaderboard button
        self.lb_button_img = ImageTk.PhotoImage(
            Image.open("Assets\LB_StartMenu.png")
        )
        self.lb_button = Button(
            self.start_frame,
            image=self.lb_button_img,
        ...
        Hovertip(
            self.lb_button, "Displays leaderboard", hover_delay=1000
        )  # 1000ms delay before showing tooltip
        self.lb_button.place(x=792, y=546, anchor=NW)

        # Settings button
        self.set_b_img = ImageTk.PhotoImage(
            Image.open("Assets\Settings_StartMenu.jpg")
        )
        self.set_b = Button(
            self.start_frame,
            image=self.set_b_img,
        ...
        Hovertip(
            self.set_b, "Opens keybinds remapping", hover_delay=1000
        )
        self.set_b.place(x=1, y=594, anchor=SW)

        # Small "How to Play" button
        self.htp_b_img = ImageTk.PhotoImage(
            Image.open("Assets\HowToPlayButton.jpg")
        )
        self.htp_b = Button(
            self.start_frame,
            image=self.htp_b_img,
            command=self.tipscreen,
            bd=0,
            highlightthickness=0,
            padx=0,
            pady=0,
        ...
        # Socials button
        url = "https://www.instagram.com/torsoq/ \
        profilecard/?igsh=NW9oY3Bjbm4xdW4="
        self.soc_b_img = ImageTk.PhotoImage(
            Image.open("Assets\SocialsButton.png")
        )
        self.soc_b = Button(
            self.start_frame,
            image=self.soc_b_img,
        ...
        Hovertip(
            self.soc_b,
            "Open socials webpage on default browser",
            hover_delay=1000,
        )
    ...
    def username_entry(self):
        ...
        self.NameImg = ImageTk.PhotoImage(
            Image.open(r"Assets\NameEntry.png").resize((643, 74))
        )
        label = Label(
            self.start_frame,
            image=self.NameImg,
    ...
    def settings(self):
        ...
        self.settings_frame = Frame(
            root, width=sW - 3, height=sH - 2, bg="black", bd=0
        )
        ...
        for action, key in self.key_binds.items():
            KeyB = Button(
                self.settings_frame,
                text=key,
                command=lambda a=action: self.Remap(a),
                font=(self.GameFont),
                bd=2,
                bg="#222a5c",
                fg="green",
            )
            # we do a=action because of pythons closure behaviour
            # in loops where lambda takes a refrence of any
            # parameter at the end of the loop then runs
        ...
        self.xb_img = ImageTk.PhotoImage(
            Image.open(r"Assets\x_button.png").resize((31, 31))
        )
        self.x_lb = Button(
            self.settings_frame,
            image=self.xb_img,
        ...
        self.clicked.set(
            options[0 if self.BossKeyTransparent == 0 else 1]
        )
        dropdown = OptionMenu(
            self.settings_frame, self.clicked, *options
        )
        dropdown.config(
            font=self.GameFont,
            bg="#222a5c",
        ...
        dropdown["menu"].config(
            font=("CyberpunkCraftpixPixel", 10),
            bg="#222a5c",
            fg="white",
            activeforeground="black",
            bd=0,
    ...
    def Remap(self, action):
        ...
        button.bind(
            "<KeyPress>",
            lambda event: self.capture_key(event, action, button),
        )
    ...
    def save_settings(self):
        ...
        """
        # its own function rather than cojoined with save_state
        # to make keybinds global to all users and not just one
    ...
    def load_settings(self):
        ...
        if os.path.exists("settings.json"):
            with open("settings.json", "r") as file:
                ...
                self.BossKeyTransparent = self.key_binds.get(
                    "bosstype", 0
                )
    ...
    def tipscreen(self):
        ...
        self.root.after(
            2500,
            lambda: (self.tipscreen.destroy(), self.block_b.destroy()),
        )
    ...
    def game(self, event=None):
        ...
        self.WaveBreakImg = ImageTk.PhotoImage(
            Image.open("Assets\WaveBreak.jpg")
        )
        ...
        self.gamebg_2 = ImageTk.PhotoImage(
            Image.open("Assets\Gamebg_2.png").resize((self.W, self.H))
        )
        self.gamebg_3 = ImageTk.PhotoImage(
            Image.open("Assets\Gamebg_3.png").resize((self.W, self.H))
        )
        self.gamebg_4 = ImageTk.PhotoImage(
            Image.open("Assets\Gamebg_4.png").resize((self.W, self.H))
        )
        self.gamebg = [
            self.gamebg_1,
            self.gamebg_2,
        ...
        self.cn.pack()
        self.GameBG = self.cn.create_image(
            0, 0, image=self.gamebg_1, anchor=NW
        )

        # Load pause option and game over menus and necessary button images
        self.OptionsImg = ImageTk.PhotoImage(
            Image.open("Assets\optionsmenu.png")
        )
        self.GameOverImg = ImageTk.PhotoImage(
            Image.open("Assets\GameOverMenu.png")
        )

        # Load and initialize player sprite 
        self.PlayerImg = ImageTk.PhotoImage(
            Image.open("Assets\PlayerIdleR.png")
        )
        self.Player_Sprite = self.cn.create_image(
            self.W // 2,
            int(self.H * 0.82),
            image=self.PlayerImg,
            anchor=CENTER,
        )
        self.JumpImg = ImageTk.PhotoImage(
            Image.open("Assets\PlayerJump\JumpRight.png").resize(
                (80, 80)
            )
        ) # set default jumping image orientation
        self.HurtR = ImageTk.PhotoImage(
            Image.open("Assets\HurtRight.png").resize((80, 80))
        )
        self.HurtL = ImageTk.PhotoImage(
            Image.open("Assets\HurtLeft.png").resize((80, 80))
        )
        self.IdleR = ImageTk.PhotoImage(
            Image.open("Assets\PlayerIdleR.png")
        )
        self.IdleL = ImageTk.PhotoImage(
            Image.open("Assets\PlayerIdleL.png")
        )
        ...
        self.ScoreTxt = self.cn.create_text(
            90, 20, text=f"Score: {self.Score}", font=self.GameFont
        )
        self.FullHealthBarImg = ImageTk.PhotoImage(
            Image.open(r"Assets\HealthBar\100.png").resize((250, 14))
        )
        self.HP = self.cn.create_image(
            400, self.H * 0.03, image=self.FullHealthBarImg, anchor=NW
        )
        self.health = 100
        self.last_hit_time = 0  # default 0 to avoid errors first time running
        self.dx = 0  # default 0 to avoid errors first time running
        self.grav = 0.1
        ...
        [
            key
            for key in [
                self.key_binds["attack"],
                self.key_binds["right"],
                self.key_binds["left"],
            ]
            if self.cn.bind(f"<KeyRelease-{key}>", self.deaction)
        ]
        ...
        self.PauseImg = ImageTk.PhotoImage(
            Image.open("Assets\Pause_Button.png")
            .convert("RGBA")
            .resize((25, 25))
        )
    ...
    def action(self, event):
        ...
        if (
            event.keysym == self.key_binds["right"]
            and self.x < self.W - 20
        ):
            if not self.mob_collisions(dx=10):
                ...
                self.JumpImg = ImageTk.PhotoImage(
                    Image.open(
                        "Assets\PlayerJump\JumpRight.png"
                    ).resize((80, 80))
                )
                if not self.Running:
                    self.Running = True
            ...
            if not self.mob_collisions(dx=-10):
                ...
                self.JumpImg = ImageTk.PhotoImage(
                    Image.open("Assets\PlayerJump\JumpLeft.png").resize(
                        (80, 80)
                    )
                )
                if not self.Running:
            ...
            event.keysym == self.key_binds["jump"] and not self.Jumping
        ) and (
            self.x <= (self.W - 10) and self.x >= 20
        ):
            # right boundary shorter than left; extra leading transparent pixels
            # (when sprite facing east)
            self.Jumping = True
            self.y_speed = -3
            ...
            if self.Jumping or self.Attacking or self.RunAttacking:
                return
                # Prevents attack if player is jumping or already attacking
            self.Attack()
    ...
    def Attack(self):
        ...
        min_distance = float(
            "inf"
        )  # To track only the closest mob to the player
        if self.Running and not self.RunAttacking:
            self.RunAttacking = True
            self.Action = True
            self.running_frames = (
                self.run_attack_right
                if self.dx > 0
                else self.run_attack_left
            )
        elif not self.Attacking:
            self.Attacking = True
            self.Action = True
            self.running_frames = (
                self.attack_right if self.dx > 0 else self.attack_left
            )
        for zombie in self.Zombies:
            if zombie.alive:
                zx, zy = self.cn.coords(zombie.zombie_sprite)
                distance = math.sqrt(
                    (self.x - zx) ** 2 + (self.y - zy) ** 2
                )
                if (
                    abs(self.x - zx) < attack_range
    ...
    def Jump(self):
        ...
        if (
            self.state == "loaded"
        ):  # to account for when save has player in mid air
            self.Jumping = True
            self.state = "game"
        ...
        if self.BossKActive:
            if self.Transparent is False:
                ...
                root.overrideredirect(
                    False
                )  # rebirth menu title bar and basically the border
                self.Transparent = False
            self.BossKActive = False
            return
        elif self.BossKeyTransparent:
            self.Transparent = True
            self.TransRec = self.cn.create_rectangle(
                0,
                0,
                self.W + 5,
                self.H + 5,
            ...
            root.overrideredirect(
                True
            )  # remove menu title bar and basically the border
        else:
            self.TranspWin = Toplevel()
            self.TranspWin.attributes("-fullscreen", True)
            self.BossKImg = ImageTk.PhotoImage(
                Image.open("Assets\BossKeyImg.png").resize(
                    (
                        self.TranspWin.winfo_screenwidth(),
                        self.TranspWin.winfo_screenheight(),
                    )
                )
            )
    ...
    def animate(self):
        ...
        if self.state == "end":
            if self.frame_index < len(self.running_frames):
                self.cn.itemconfig(
                    self.Player_Sprite,
                    image=self.running_frames[self.frame_index],
                )
                self.frame_index += 1
                ...
                self.cn.itemconfig(
                    self.Player_Sprite,
                    image=self.running_frames[self.frame_index - 1],
                )
                return
        elif self.state == "lb":
            ...
            self.lbcn.itemconfig(
                self.lbbg, image=self.running_frames[self.frame_index]
            )
            self.delay = 4000 if self.frame_index % 2 == 0 else 100
            self.root.after(self.delay, self.animate)

        if not self.Action or self.state != "game":
            return
        self.bounds()
    ...
    def check_cheat_code(self, key):
        ...
        if (
            self.key_sequence[-len(self.konami_sequence):]
            == self.konami_sequence
        ):
            self.cheat_code()
            self.key_sequence.clear()
            # or we can do self.key_sequence = [] to clear the list
    ...
    def cheat_code(self):
        ...
        """
        self.cheaton = True  # for modularity later
        if self.grav >= 0.05:
            self.grav -= 0.03
        elif self.grav >= 0.03:
            self.grav -= 0.01
    ...
    def update_score(self):
        ...
        self.cn.itemconfig(
            self.ScoreTxt, text=f"Score: {self.Score}"
        )  # self.ScoreTxt.config doesnt work
        # (cause its on a canvas and not a standard widget)
        if (
            self.RelativeScore % self.scorepwave == 0
            and self.Score != 0
        ):
            self.WaveNum += 1
            self.scorepwave += 1
            ...
            self.typewriter()
            self.root.after(
                self.delta * len(self.WaveMsg), self.start_wave
            )
    ...
    def start_wave(self):
        ...
        self.cn.create_window(20, 20, window=self.Pause_button)
        self.cn.itemconfig(
            self.GameBG, image=self.gamebg[(self.WaveNum % 4) - 1]
        )
        self.mob_cap = False
        self.spawn_mobs()
        self.action_mobs()
    ...
    def typewriter(self):
        ...
        self.WaveMsg += 43*" "
        self.WaveTxt = self.cn.create_text(
            self.W // 2,
            self.H * 0.35,
            text="",
            font=self.InputFont,
        ...
        for i in range(len(self.WaveMsg) + 1):
            s = self.WaveMsg[:i]
            updatedtxt = lambda s=s: self.cn.itemconfig(
                self.WaveTxt, text=s
            )
    ...
    def reset_sprite(self):
        ...
        self.cn.itemconfig(
            self.Player_Sprite,
            image=self.IdleR if self.dx > 0 else self.IdleL,
        )
    ...
    def save_state(self):
        ...
        """
        print("Saved "+self.username+"'s game state at: ", end="")
        print(datetime.datetime.now().strftime("%H:%M:%S %d-%m-%Y"))
        self.state = "paused"
        self.state_file = os.path.join(
            self.saves_folder, f"{self.username}_game_state.json"
        )
        for zombie in self.Zombies:
            if not zombie.alive:
                ...
                )  # Removes dead zombies from the list first
        self.game_data = {
            "player": {
                "name": self.username,
    ...
    def load_state(self):
        ...
        """
        if os.path.exists(
            f"PlayerSaves\{self.username}_game_state.json"
        ):
            self.state_file = (
                f"PlayerSaves\{self.username}_game_state.json"
            )
            ...
            if self.WaveNum != 0:
                self.cn.itemconfig(
                    self.GameBG,
                    image=self.gamebg[(self.WaveNum % 4) - 1],
                )
    ...
    def leaderboard(self):
        ...
        self.lb_frame = Frame(
            root, width=600, height=300, bg="#ab23ff", bd=0
        )
        self.lb_frame.place(
        ...
        self.lbcn.pack()
        self.lbbg = self.lbcn.create_image(
            0, 0, image=self.running_frames[0], anchor=NW
        )
        # X button to leave leaderboard
        self.xb_img = ImageTk.PhotoImage(
            Image.open(r"Assets\x_button.png")
        )
        self.x_lb = Button(
            self.lb_frame,
            image=self.xb_img,
            command=self.CloseLB,
            bd=0,
            highlightthickness=0,
            padx=0,
            pady=0,
        ...
        display_scores(
            self.lb_frame, self.lbcn, self.GameFont, self.LBFont
        )
        self.animate()
    ...
    def pause(self):
        ...
        if self.state == "game":
            ...
            self.OptionsButton = Button(
                image=self.OptionsImg,
                borderwidth=0,
                highlightthickness=0,
                background="#ab23ff",
            )
            self.OptionsButton.bind("<Button-1>", self.Optionclicked)
            self.OptionsMenu = self.cn.create_window(
                self.W // 2,
                int(self.H * 0.5),
                window=self.OptionsButton,
                anchor=CENTER,
            )

            # Pausing mob related stuff
            self.root.after_cancel(self.spawn_timer)
            # calculating time for the next spawn
            self.remaining_time = max(
                0,
                (self.spawn_interval / 1000)
                - (time.time() - self.elapsed_spawn_time),
            )
            for zombie in self.Zombies:
                zombie.pauseani()
            self.root.after_cancel(self.mobact_timer)
    ...
    def resume(self):
        ...
        reversing all actions done in the pause method.
        """
        if self.state == "paused":
            ...
            if self.zombie_num < self.scorepwave:
                if self.remaining_time > 0:
                    self.spawn_timer = self.root.after(
                        int(self.remaining_time * 1000), self.spawn_mobs
                    )
                else:
                    self.spawn_mobs()
                self.action_mobs()
    ...
    def Optionclicked(self, event):
        ...
        x, y = event.x, event.y
        if self.state == "paused":
            # X button dimensions x1=405 y1=56 x2=425 y2=77
            # then resume;restart;stats;settings;quit
            dimen = [
                [174, 3, 192, 22],
                [14, 39, 178, 71],
                [16, 85, 178, 116],
            ...
            dimen = [
                [37, 89, 73, 127],  # quit
                [96, 88, 133, 125],  # leaderboard
                [156, 88, 194, 125]  # restart
            ]
            for d in dimen:
                if (x > d[0] and x < d[2]) and (y > d[1] and y < d[3]):
                    b = dimen.index(d)
    ...
    def game_over(self):
        ...
        self.dmsgs = [
            f"{self.username} was eviscerated.",
            f"{self.username} was brutally dissected.",
            f"{self.username} had both kidneys stolen.",
            f"{self.username}'s body was donated to science.",
            f"{self.username} received a forced amputation.",
            f"{self.username} was voluntold to donate blood.",
            f"{self.username} suffered from a tragic lobotomy.",
            f"{self.username}'s body was donated to science.",
        ]
        for zombie in self.Zombies:
            zombie.changestate("idle")
        self.running_frames = (
            self.dead_right if self.dx > 0 else self.dead_left
        )
        self.state = "end"
        self.root.after_cancel(self.spawn_timer)
        self.root.after_cancel(self.mobact_timer)
        self.animate()
        self.Pause_button.destroy()
        self.deathmsg = self.cn.create_text(
            self.W // 2,
            230,
            text=self.dmsgs[random.randint(0, len(self.dmsgs) - 1)],
            font=self.GameFont,
            anchor=CENTER,
            fill="#E11919",
            tags="death_msg",
        ...
        for i in range(3):
            self.cn.create_rectangle(
                ...
                stipple="gray50",
                tags="bg",
            )  # Stipple pattern for transparent txt bg
        self.cn.tag_raise(
            "death_msg", "bg"
        )  # to display text on top of transparent bg
        self.OverButton = Button(
            image=self.GameOverImg,
            borderwidth=0,
            highlightthickness=0,
            background="#ab23ff",
        )
        self.OverButton.bind("<Button-1>", self.Optionclicked)
        self.GameOverMenu = self.cn.create_window(
            self.W // 2,
            int(self.H * 0.3),
            window=self.OverButton,
            anchor=CENTER,
        )
    ...
    def spawn_mobs(self):
        ...
        x = random.choice(
            [
                random.randint(0, int(0.25 * float(self.W))),
                random.randint(int(0.75 * float(self.W)), self.W),
            ]
        )  # random position from either 1st/4th map quarter
        y = int(self.H * 0.82)
        zombie = NPC(
            self,
            self.cn,
        ...
        # Schedules next spawning
        # (after spawn_interval miliseconds)
        self.spawn_timer = self.root.after(
            self.spawn_interval, self.spawn_mobs
        )
    ...
    def take_damage(self):
        ...
        ctime = time.time()
        if (
            ctime - self.last_hit_time >= 2
        ):  # 2 second cooldown for damaging the player
            self.health -= 10
            self.cn.itemconfig(
                self.Player_Sprite,
                image=self.HurtR if self.dx > 0 else self.HurtL,
            )
            self.root.after(
                200, self.reset_sprite
            )  # adds 200 ms delay before undisplaying the hurt sprite
            if self.health < 0 and self.state != "end":
                ...
                )  # tkinter will garbage collect
                # if i don't attribute to the class
                self.cn.itemconfig(self.HP, image=self.newhealthimg)
                self.last_hit_time = ctime

    def action_mobs(self):
        """
        Manage zombie behavior and movement during gameplay;
    ...
    def mob_collisions(self, dx):
        """
        Detect and prevent player movement through zombie,
         providing collision feedback.
        ...
        for zombie in self.Zombies:
            ...
            if abs(next_px - zx) < 40:
                if abs(py - zy) < 20:
                    # Block movement only towards the mob
                    # (to allow player to escape in opp direction)
                    towards_mob = (dx > 0 and next_px < zx) or (
                        dx < 0 and next_px > zx
                    )
                    if towards_mob:
                        return True
...
class NPC(App):
    ...
    Sets up zombie's initial state, animations,
    ...
    def __init__(
        ...
        self.frame_index = 0
        self.dx = 0
        self.IdleZ1 = ImageTk.PhotoImage(
            Image.open(r"Assets\NPC\Mob1Idle.png")
        )
        self.zombie_sprite = self.cn.create_image(
            x, y, image=self.IdleZ1, anchor=CENTER
        )
        self.alive = True
        self.mob_speed = 3
        self.health = 1
    ...
    def changestate(self, new_state):
        """
        Change the zombie's current state
        and animation frames based on game context.
        """
    ...
    def NextWave(self):
        ...
        # every round changes
        self.mob_speed += 1
        self.app.spawn_interval -= (
            500 if self.app.spawn_interval > 2000 else 0
        )
    ...
    def animate(self):
        ...
        updates sprite image, and
        handles special cases like death animation
        and sprite deletion.
        """
        ...
        if (
            not self.alive
        ):  # Deletes dead zombie after death animation ends
            if self.frame_index == len(self.frames) - 1:
                self.cn.delete(self.zombie_sprite)
                return
        self.frame_index = (self.frame_index + 1) % len(self.frames)
        self.cn.itemconfig(
            self.zombie_sprite, image=self.frames[self.frame_index]
        )
    ...
    def moveto(self, playerx):
        ...
        if (
            abs(self.dx) > 10
        ):  # Min x from p to avoid endless switching if close
            direction = (
                "walking_right" if self.dx > 0 else "walking_left"
            )
            self.changestate(direction)
            step = (
                min(self.mob_speed, self.dx)
                if self.dx > 0
                else max(-self.mob_speed, self.dx)
            )
            self.cn.move(self.zombie_sprite, step, 0)

    def collisions(self, playerx, playery, attack_range=30):
        """
        Determine if zombie is in range and attack if so.

        Args:
            playerx: Player's x-coordinate
            playery: Player's y-coordinate
            attack_range : Maximum distance for attack. Defaults to 30.
        ...
        if (
            abs(playerx - x) < attack_range
            and abs(playery - y) < attack_range - 10
        ):
            if not self.state.startswith("attacking"):
                self.changestate(
                    f"attacking_{'right' if self.dx>0 else 'left'}"
                )
            return True
        else:
            return False
    ...
    def take_damage(self, damage):
        ...
        Reduces zombie health,
        triggers death sequence and animation if no HP left,
        and updates game score accordingly.
        """
        self.health -= damage
        if self.health <= 0:
            self.alive = False
